...
// Guardar los valores
$email = $data['email'];
$moca_recuerdo_diferido = $data['moca_recuerdo_diferido'];
...
if ($stmt_check->num_rows > 0) {
    // Si el correo existe, actualizar la columna moca_recuerdo_diferido
    $sql_update = "UPDATE consentimiento_f SET moca_recuerdo_diferido = ? WHERE correo = ?";
    $stmt_update = $conn->prepare($sql_update);
    $stmt_update->bind_param("ss", $moca_recuerdo_diferido, $email);
