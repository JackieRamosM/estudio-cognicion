...
function startTimer() {
    timer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
			...
			window.location.href = "stroop_fase3_instrucciones.html";
        }
    }, 1000);
}
...
async function fetchColors() {
    try {
        const response = await fetch('stroop_test_p2.php');
        if (!response.ok) {
...
// Función para mostrar 'XXXX' con el color
function showNextColor() {
    ...
    // Cambiar el color de las 'XXXX' usando el color de la base de datos
